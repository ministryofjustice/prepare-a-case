...
module.exports = {
  ...
  markMatches: (matchString, sourceString) => {
    ...
    const filteredArr = matchString.split(' ').map(item => {
      ...
      return (
        (sourceSplit.includes(item.toLowerCase())
          ? `<mark>${item}</mark>`
          : item) + (hasComma ? ',' : '')
      )
    })
  ...
  courtRoomDisplay: sourceString => {
    return sourceString.includes('Courtroom')
      ? sourceString.replace(/([A-Za-z 0]*)?/, '')
      : sourceString.replace(/([0]*)?/, '')
  },
  ...
  hearingOutcomeResultedDateFormat: dateString => {
    if (!dateString) {
      return 'Not available'
    }
    return moment(dateString).format('D MMM YYYY \\at HH:mm')
  },
