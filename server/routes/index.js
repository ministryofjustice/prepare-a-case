...
module.exports = function Index ({ authenticationMiddleware }) {
  ...
  router.get('/:courtCode/hearing/:hearingId/defendant/:defendantId/record', defaults, async (req, res) => {
    ...
    const psrRequestedConvictions = communityResponse.convictions?.filter(conviction => conviction.active && conviction.awaitingPsr && conviction.psrReports && conviction.psrReports.length)
      .map(conviction => {
    ...
    templateValues.data.psrRequestedConvictions = psrRequestedConvictions
