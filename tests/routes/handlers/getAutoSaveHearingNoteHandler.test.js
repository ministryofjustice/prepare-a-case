...
describe('getAutoSaveHearingNoteHandler', () => {
  ...
  const subject =
    require('../../../server/routes/handlers/getAutoSaveHearingNoteHandler')(
      caseServiceMock
    )
  ...
  it('should invoke save draft note and return response', async () => {
    caseServiceMock.saveDraftHearingNote.mockResolvedValueOnce({
      data: { note: 'note' },
      status: 200
    })
    // When
    ...
    // Then
    expect(caseServiceMock.saveDraftHearingNote).toHaveBeenLastCalledWith(
      testHearingId,
      'A note',
      'Adam Sandler',
      'test-defendant-id'
