...
describe('getDeleteHearingNoteHandler', () => {
  ...
  const subject =
    require('../../../server/routes/handlers/getDeleteHearingNoteHandler')(
      caseServiceMock
    )
  ...
  it('should invoke delete hearing note api and redirect to case summary delete success notification', async () => {
    ...
    // Then
    expect(mockRequest.session.deleteHearingNoteSuccess).toEqual(
      targetHearingId
    )
    expect(caseServiceMock.deleteHearingNote).toHaveBeenCalledWith(
      targetHearingId,
      noteId,
      'test-defendant-id'
    )
