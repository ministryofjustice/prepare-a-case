...
describe('Case service', () => {
  ...
  it('should call the API to request case list data', async () => {
    moxios.stubRequest(`${apiUrl}/court/SHF/cases?date=2020-01-01&VERSION2=true`, {
      status: 200,
      response: {
        cases: []
      }
    ...
    expect(moxios.requests.mostRecent().url).toBe(
      `${apiUrl}/court/SHF/cases?date=2020-01-01&VERSION2=true`
    )
    return response
  })
  ...
  it('should return error response when API call to get case list fails with 500 status', async () => {
    const endpoint = `${apiUrl}/court/SHF/cases?date=2020-01-01&VERSION2=true`
    moxios.stubRequest(endpoint, {
      status: 500
    })
