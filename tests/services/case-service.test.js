...
describe('Case service', () => {
  ...
  describe('Files', () => {
    it('should delete a file', async () => {
      moxios.stubRequest(`${apiUrl}/hearing/hearingId/defendant/defendantId/files/fileId`, {
        status: 200,
        response: {
          cases: []
        }
      ...
      expect(moxios.requests.mostRecent().url).toBe(`${apiUrl}/hearing/hearingId/defendant/defendantId/files/fileId`)
      return response
    })
    ...
    it('should upload a file', () => {
      ...
      proxy.mockImplementationOnce((url, config) => {
        ...
        expect(config.proxyReqPathResolver()).toBe('/hearing/hearingId/defendant/defendantId/files')
        ...
        // ===200
        expect(config.userResDecorator(mockProxyRes, mockProxyData)).toBe(mockJson)

        // bad json
        mockProxyData = Buffer.from('{[dd')
        expect(() => config.userResDecorator(mockProxyRes, mockProxyData)).toThrow()
        ...
        expect(config.userResDecorator(mockProxyRes, mockProxyData)).toBe(mockProxyData)
      ...
      files.post(mockReq, mockRes, mockNext, responseFormatter, 'hearingId', 'defendantId')
    })
    ...
    it('should download a file', async () => {
      ...
      proxy.mockImplementationOnce((url, config) => {
        expect(url).toBe(apiUrl)
        expect(config.proxyReqPathResolver()).toBe('/hearing/hearingId/defendant/defendantId/files/fileId/raw')
      ...
      files.getRaw(mockReq, mockRes, mockNext, skipper, 'hearingId', 'defendantId', 'fileId')
    })
  })
