...
Then('I should have a button group with a button labeled {string}', $label => {
    cy.get('.govuk-button-group.button-group').should('exist')
    cy.get('.govuk-button-group.button-group').within(() => {
        cy.get('.govuk-button.action-button').contains($label).should('exist')
    })
})
